...
def load_fuzzy(npz_path):
    ...
    step = int(np.ceil((Z.size / MAX_POINTS) ** 0.5))
    if step > 1:
        Z = Z[::step, ::step]
...
def main():
    ...
    ap.add_argument("--png",   default=None, help="Optional PNG output")   # <— ADD THIS LINE
    ...
    ap.add_argument("--gzip", action="store_true",
                help="Also write a gzipped HTML alongside the normal file")
    ap.add_argument("--gzip-out", default=None,
                help="Custom path for gzipped HTML (optional)")
    args = ap.parse_args()
    ...
    Z = fuzzy["Z"]
    approx_points = int(Z.size)
    if approx_points > 2_000_000 and int(args.downsample) == 1:
        # small visual impact, big size drop
        args.downsample = 2
    ...
    logging.info("Plot generated, writing to HTML")
    # fig.write_html(args.out, include_plotlyjs="cdn", full_html=True)
    fig.write_html(
    args.out,
    include_plotlyjs="cdn",         # was "include_plotlyjs=True" or default
    full_html=True,
    config={"responsive": True, "displaylogo": False},
)

    logging.info(f"HTML written to {args.out}")
    ...
    if args.gzip or args.gzip_out:
        ...
        try:
            ...
            logging.info(f"Gzipped HTML written to {gz_path} ({size_mb:.2f} MB)")
        except Exception:
            logging.info(f"Gzipped HTML written to {gz_path}")
    ...
    if args.png:
        try:
            ...
            logging.warning(f"PNG export failed (continuing with HTML): {e}")
